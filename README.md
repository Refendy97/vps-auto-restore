...
Script ini digunakan untuk **RESTORE PENUH VPS** yang menjalankan:
- Marzban Panel (Docker)
- Xray / VPN (database user & traffic)
- Nginx
- Bot Telegram

Restore berbasis **backup Google Drive (rclone)**.  
Script mendukung **Debian 11 & Debian 12 otomatis** (tanpa command berbeda).

---

## 📦 DATA YANG DIRESTORE
Restore mengikuti isi file:

`/opt/vpn-backup/backup.env` (source of truth)

Contoh data yang direstore:
- `/var/lib/marzban`
- `/opt/marzban`
- `/opt/marzban-bot`
- `/etc/nginx`
